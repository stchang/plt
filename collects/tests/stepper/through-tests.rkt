...
;     :: {(! (+ 3 4))} -> {7})
  ...
  (t 'lazy-top-level-force1 m:lazy
     (+ 2 3)
     :: {(+ 2 3)}
     -> {5})
  
  (let ([def '(define (f x) (+ x 1))])
    (t 'lazy-delay-arg1 m:lazy
       ,def (f (+ 2 3))
       :: ,def {(f (+ 2 3))}
;       -> ,def {((lambda (x) (+ x 1)) (+ 2 3))}
;       :: ,def {((lambda (x) (+ x 1)) (+ 2 3))}
       -> ,def {(+ (+ 2 3) 1)}
       :: ,def (+ {(+ 2 3)} 1)
       -> ,def (+ {5} 1)
  ...
  (let ([def '(define (f x) (+ x (+ x x)))])
    (t 'lazy-parallel-arg-reduce1 m:lazy
       ,def (f (+ 1 2))
       :: ,def {(f (+ 1 2))}
;       -> ,def {((lambda (x) (+ x (+ x x))) (+ 1 2))}
;       :: ,def {((lambda (x) (+ x (+ x x))) (+ 1 2))}
       -> ,def {(+ (+ 1 2) (+ (+ 1 2) (+ 1 2)))}
       :: ,def (+ {(+ 1 2)} (+ {(+ 1 2)} {(+ 1 2)}))
       -> ,def (+ {3} (+ {3} {3}))
  ...
  (let ([def '(define (f x) (+ (+ x x) x))])
    (t 'lazy-parallel-arg-reduce2 m:lazy
       ,def (f (+ 3 4))
       :: ,def {(f (+ 3 4))}
;       -> ,def {((lambda (x) (+ (+ x x) x)) (+ 3 4))}
;       :: ,def {((lambda (x) (+ (+ x x) x)) (+ 3 4))}
       -> ,def {(+ (+ (+ 3 4) (+ 3 4)) (+ 3 4))}
       :: ,def (+ (+ {(+ 3 4)} {(+ 3 4)}) {(+ 3 4)})
       -> ,def (+ (+ {7} {7}) {7})
  ...
  (let ([def1 '(define (f x) (+ x (+ x x)))]
        [def2 '(define (g x) (+ (+ x x) x))]
        [app1 '(f (+ 1 2))]
        [app2 '(g (+ 3 4))])
    (t 'lazy-parallel-arg-reduce3 m:lazy
       ,def1 ,def2 (+ ,app1 ,app2)
       :: ,def1 ,def2 (+ {,app1} ,app2)
;       :: ,def1 ,def2 (+ {((lambda (x) (+ x (+ x x))) (+ 1 2))} ,app2)
       -> ,def1 ,def2 (+ {(+ (+ 1 2) (+ (+ 1 2) (+ 1 2)))} ,app2)
       :: ,def1 ,def2 (+ (+ {(+ 1 2)} (+ {(+ 1 2)} {(+ 1 2)})) ,app2)
       -> ,def1 ,def2 (+ (+ {3} (+ {3} {3})) ,app2)
       :: ,def1 ,def2 (+ (+ 3 {(+ 3 3)}) ,app2) -> ,def1 ,def2 (+ (+ 3 {6}) ,app2)
       :: ,def1 ,def2 (+ {(+ 3 6)} ,app2)       -> ,def1 ,def2 (+ {9} ,app2)
;       :: ,def1 ,def2 (+ 9 {((lambda (x) (+ (+ x x) x)) (+ 3 4))})
       :: ,def1 ,def2 (+ 9 {,app2})
       -> ,def1 ,def2 (+ 9 {(+ (+ (+ 3 4) (+ 3 4)) (+ 3 4))})
       :: ,def1 ,def2 (+ 9 (+ (+ {(+ 3 4)} {(+ 3 4)}) {(+ 3 4)}))
       -> ,def1 ,def2 (+ 9 (+ (+ {7} {7}) {7}))
       :: ,def1 ,def2 (+ 9 (+ {(+ 7 7)} 7)) -> ,def1 ,def2 (+ 9 (+ {14} 7))
       :: ,def1 ,def2 (+ 9 {(+ 14 7)})      -> ,def1 ,def2 (+ 9 {21})
       :: ,def1 ,def2 {(+ 9 21)}            -> ,def1 ,def2 {30}
       ))
  
  (let* ([def '(define (f x) (+ x x))]
         [lam '(lambda (x) (+ x x))]
         [app1 '(+ 1 2)]
         [app2 `(f ,app1)]
         [app3 `(f ,app2)])
    (t 'lazy-parallel-arg-reduce-double-apply m:lazy
       ,def ,app3
       :: ,def {,app3}
;       -> ,def {(,lam ,app2)}
;       :: ,def {(,lam ,app2)}
       -> ,def {(+ ,app2 ,app2)}
;       :: ,def (+ {(,lam ,app1)} {(,lam ,app1)})
       :: ,def (+ {,app2} {,app2})
       -> ,def (+ {(+ ,app1 ,app1)} {(+ ,app1 ,app1)})
       :: ,def (+ (+ {,app1} {,app1}) (+ {,app1} {,app1}))
       -> ,def (+ (+ {3} {3}) (+ {3} {3}))
       :: ,def (+ {(+ 3 3)} {(+ 3 3)})
       -> ,def (+ {6} {6})
  ...
  (t 'lazy-cons1 m:lazy
     ...
     -> {3})
  (t 'lazy-cons2 m:lazy
     (cadr (cdr (cddr (cons 1 (cons 2 (cons 3 (cons 4 (cons 5 (cons 6 empty)))))))))
     :: (cadr (cdr {(cddr (cons 1 (cons 2 (cons 3 (cons 4 (cons 5 (cons 6 empty)))))))}))
     -> (cadr (cdr {(cons 3 (cons 4 (cons 5 (cons 6 empty))))}))
     :: (cadr {(cdr (cons 3 (cons 4 (cons 5 (cons 6 empty)))))})
     -> (cadr {(cons 4 (cons 5 (cons 6 empty)))})
     :: {(cadr (cons 4 (cons 5 (cons 6 empty))))}
     -> {5})
  (t 'lazy-cons3 m:lazy
     (cadr (cdr (cddr (cons 1 (cons 2 (cons 3 (cons 4 (cons 5 (cons 6 7)))))))))
     :: (cadr (cdr {(cddr (cons 1 (cons 2 (cons 3 (cons 4 (cons 5 (cons 6 7)))))))}))
     -> (cadr (cdr {(cons 3 (cons 4 (cons 5 (cons 6 7))))}))
     :: (cadr {(cdr (cons 3 (cons 4 (cons 5 (cons 6 7)))))})
     -> (cadr {(cons 4 (cons 5 (cons 6 7)))})
     :: {(cadr (cons 4 (cons 5 (cons 6 7))))}
     -> {5})
  ...
  (t 'lazy-list6 m:lazy
    ...
    (t 'lazy-take1 m:lazy
       ...
       :: (* 2 (second {(take 2 (list 1 2 3 4 5))}))
       -> (* 2 (second {(cons 1 <DelayedEvaluation#0>)}))
       :: (* 2 {(second (cons 1 <DelayedEvaluation#0>))})
       -> (* 2 {2})
       :: {(* 2 2)}
       -> {4})
    (t 'lazy-take2 m:lazy
       (* 2 (cadr (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))))
       :: (* 2 (cadr {(take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))}))
       -> (* 2 (cadr {(cons (+ 1 2) <DelayedEvaluation#0>)}))
       :: (* 2 {(cadr (cons (+ 1 2) <DelayedEvaluation#0>))})
       -> (* 2 {(+ 3 4)})
       -> (* 2 {7})
       :: {(* 2 7)}
       -> {14})
    (let ([def '(define (f x) (* 2 (cadr x)))])
      (t 'lazy-take3 m:lazy
         ,def (f (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6))))
         :: ,def {(f (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6))))}
         -> ,def {(* 2 (cadr (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))))}
         :: ,def (* 2 (cadr {(take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))}))
         -> ,def (* 2 (cadr {(cons (+ 1 2) <DelayedEvaluation#0>)}))
         :: ,def (* 2 {(cadr (cons (+ 1 2) <DelayedEvaluation#0>))})
         -> ,def (* 2 {(+ 3 4)})
         -> ,def (* 2 {7})
         :: ,def  {(* 2 7)}
    ...
    (let ([def '(define (f x) (+ (car x) (car x)))])
      (t 'lazy-take4 m:lazy
         ,def (f (take 2 (list 1 2 3)))
         :: ,def {(f (take 2 (list 1 2 3)))}
         -> ,def {(+ (car (take 2 (list 1 2 3))) 
                     (car (take 2 (list 1 2 3))))}
         :: ,def (+ (car {(take 2 (list 1 2 3))}) 
                    (car {(take 2 (list 1 2 3))}))
         -> ,def (+ (car {(cons 1 <DelayedEvaluation#0>)}) 
                    (car {(cons 1 <DelayedEvaluation#0>)}))
         :: ,def (+ {(car (cons 1 <DelayedEvaluation#0>))} 
                    (car (cons 1 <DelayedEvaluation#0>)))
         -> ,def (+ {1}
                    (car (cons 1 <DelayedEvaluation#0>)))
    ...
    (let ([def '(define (f x) (+ x x))])
      (t 'lazy-take6 m:lazy
         ,def (f (cadr (take 2 (list 1 2 3))))
         :: ,def {(f (cadr (take 2 (list 1 2 3))))}
         -> ,def {(+ (cadr (take 2 (list 1 2 3))) 
                     (cadr (take 2 (list 1 2 3))))}
         :: ,def (+ (cadr {(take 2 (list 1 2 3))}) 
                    (cadr {(take 2 (list 1 2 3))}))
         -> ,def (+ (cadr {(cons 1 <DelayedEvaluation#0>)}) 
                    (cadr {(cons 1 <DelayedEvaluation#0>)}))
         :: ,def (+ {(cadr (cons 1 <DelayedEvaluation#0>))} 
                    {(cadr (cons 1 <DelayedEvaluation#0>))})
         -> ,def (+ {2}
                    {2})
    ...
    (let ([def '(define (f x) (+ x x))])
      (t 'lazy-take7 m:lazy
         ,def (f (cadr (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))))
         :: ,def {(f (cadr (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))))}
         -> ,def {(+ (cadr (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))) 
                     (cadr (take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))))}
         :: ,def (+ (cadr {(take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))}) 
                    (cadr {(take 2 (list (+ 1 2) (+ 3 4) (+ 5 6)))}))
  ...
  (let ([def '(define (f x y) (cadr (list x y x)))])
    (t 'lazy-list-fn1 m:lazy
       ,def (f (/ 1 0) (+ 1 2))
       :: ,def {(f (/ 1 0) (+ 1 2))}
       -> ,def {(cadr (list (/ 1 0) (+ 1 2) (/ 1 0)))}
       -> ,def {(+ 1 2)}
       -> ,def {3}))
  (let ([def '(define (f x) (+ (cadr x) (cadr x)))])
    (t 'lazy-list-fn2 m:lazy
       ,def (f (list 1 2))
       :: ,def {(f (list 1 2))}
       -> ,def {(+ (cadr (list 1 2))
                   (cadr (list 1 2)))}
  ...
  (let ([def '(define (f x y) (+ x (car (list x y))))])
    (t 'lazy-list-fn-parallel-reduce1 m:lazy
       ,def (f (+ 1 2) (/ 1 0))
       :: ,def {(f (+ 1 2) (/ 1 0))}
       -> ,def {(+ (+ 1 2) (car (list (+ 1 2) (/ 1 0))))}
       :: ,def (+ {(+ 1 2)} (car (list {(+ 1 2)} (/ 1 0))))
       -> ,def (+ {3} (car (list {3} (/ 1 0))))
       :: ,def (+ 3 {(car (list 3 (/ 1 0)))})
       -> ,def (+ 3 {3})
       :: ,def {(+ 3 3)}
       -> ,def {6}))
  
  (t 'lazy-multi-value m:lazy
     (+ 1 2) (+ 3 4)
     :: {(+ 1 2)} -> {3}
     :: 3 {(+ 3 4)} -> 3 {7})
