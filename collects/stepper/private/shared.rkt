  ...
  (define-values (closure-table-put! closure-table-lookup in-closure-table)
    (let ([closure-table (make-weak-hash)])
	 ...
	 (hash-set! closure-table key value)
         #;(printf "putting to closure table (size = ~a): ~a = ~a\n" 
                 (hash-count closure-table) key value)
	 key)                                  ; this return allows a run-time-optimization
       (lambda args ; key or key & failure-thunk
         #;(printf "looking up in closure table (size = ~a): ~a\n" 
                 (hash-count closure-table) (car args))
         #;(let-values ([(f? f) (sequence-generate (in-hash-keys closure-table))])
           (printf "closure table keys: ~a\n" (if (f?) (f) '())))
