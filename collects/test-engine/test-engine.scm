...
(define test-display-textual%
  (class* object% ()
    ...
    ; In Windows, sometimes paths contain tildes when truncated and this can 
    ; cause printf to error because of unrecognized escape chars
    ; example: C:\Docume~1\
      ...
      (let ([src-file car]
        ...
        (string-append
         (cond [(symbol? (src-file src)) " At "]
               [(path? (src-file src))
                (string-append " In " (path->string-no-tilde (src-file src)) " at ")]
               [else " At "])
